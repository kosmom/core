...
class mail{
	...
	static function AddStringAttachment($text,$totalFilename,$db=''){
		if ($db=='')$db=core::$data['mail'];
		if (empty(self::$mails[$db]))self::connect($db);
		self::$mails[$db]->AddStringAttachment($text,$totalFilename);
	}
	...
	private static function connect($db){
		...
		if (!self::$mail_config[$db])throw new \Exception('mail connection "'.$db.'" not exists');
			...
			case 'apache':
				...
				break;
			default :
				throw new \Exception('m error in config files not match '.$db);
		}
	}
}