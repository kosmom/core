...
class form_bootstrap5{
	...
	function renderFieldFormGroupBegin($item){
		...
		$baseGroup='mb-3';
		if ($this->form->prop['type']=='form-horiontal')$baseGroup.=' row';
		if (\in_array($item['type'],array('checkbox','boolean','radio')))$baseGroup='form-check';
	...
	private function inputGroupPrefix($item){
		...
		return '<div class="input-group">'.(!empty($item['prefix'])?'<span class="input-group-text">'.$item['prefix'].'</span>':'');
	}
	private function inputGroupPostfix($item){
		if (empty($item['prefix']) && empty($item['postfix']))return '';
		return (!empty($item['postfix'])?'<span class="input-group-text">'.$item['postfix'].'</span>':'').'</div>';
	}
		...
		switch ($item['type']){
			...
			case 'select':
				...
				switch ($item['render']){
					...
					case 'select':
						$out.='<select class="form-select'.$classes.'" name="'.$renderName.($multiple?'[]':'').'"'.($multiple?' multiple':'').' '.$attributes.$required.'>';
						if ($required){
					...
					case 'check':
						if ($multiple){
							foreach ($item['values'] as $key=>$val){
								$out.='<div class="form-check"><label><input class="form-check-input" '.$attributes.' name="'.$renderName.'[]"'.(isset($valArr[$key])?' checked':'').' type="checkbox" value="'.\c\input::htmlspecialchars($key).'">'.(@$item['html']?$val:\c\input::htmlspecialchars($val)).'</label></div>';
							}
						}else{
							foreach ($item['values'] as $key=>$val){
								$out.='<div class="form-check"><label><input class="form-check-input" '.$attributes.' name="'.$renderName.'"'.($renderValue==$key?' checked':'').' type="checkbox" value="'.\c\input::htmlspecialchars($key).'">'.(@$item['html']?$val:\c\input::htmlspecialchars($val)).'</label></div>';
							}
						}
						break;
					case 'radio':
						foreach ($item['values'] as $key=>$val){
							$out.='<div class="form-check"><label><input class="form-check-input" '.$attributes.$required.' name="'.$renderName.'"'.($renderValue==$key?' checked':'').' type="radio" value="'.\c\input::htmlspecialchars($key).'">'.(@$item['html']?$val:\c\input::htmlspecialchars($val)).'</label></div>';
						}
					...
					if ($multiple){
						foreach ($item['values'] as $key=>$val){
							...
							$out.='<label class="form-check-label"><input class="form-check-input" '.$attributes.' name="'.$renderName.'[]"'.(isset($valArr[$key])?' checked':'').' type="checkbox" value="'.\c\input::htmlspecialchars($key).'">'.(@$item['html']?$val:\c\input::htmlspecialchars($val)).'</label>';
							$out.='</div>';
						}
					}else{
						foreach ($item['values'] as $key=>$val){
							$out.='<div class="form-check form-check-inline">';
							$out.='<label class="form-check-label"><input class="form-check-input" '.$attributes.' name="'.$renderName.'"'.($renderValue==$key?' checked':'').' type="checkbox" value="'.\c\input::htmlspecialchars($key).'">'.(@$item['html']?$val:\c\input::htmlspecialchars($val)).'</label>';
							$out.='</div>';
						}
					}
						break;
					...
					foreach ($item['values'] as $key=>$val){
						$out.='<div class="form-check form-check-inline">';
						$out.='<label class="form-check-label"><input class="form-check-input" '.$attributes.$required.' name="'.$renderName.'"'.($renderValue==$key?' checked':'').' type="radio" value="'.\c\input::htmlspecialchars($key).'">'.(@$item['html']?$val:\c\input::htmlspecialchars($val)).'</label>';
						$out.='</div>';
					}
						break;
					case 'fancytree':
