...
class dbwork{
		...
		switch(db::type($db)){
			case 'mysql':
				...
				$tablename=\strtolower($tablename);
                                $sql='show create table `'.($schema?$schema.'`.`'.$tablename:$tablename).'`';
				$rs=db::ea1($sql,'',$db);
				$createtable=\explode("\n",$rs['Create Table']);
	...
	// parsing to words
					if (\substr($string,0,1) == '`'){
						...
						if ($typerangepos){
							...
							if ($type=='set' || $type=='enum')$data[$column]['typerange']=65335;
						}
	...
	//echo $default.' - ';
			...
			case 'oracle':
				...
				if ($schema){
					...
					$sql="select atc.column_name,atc.data_type,data_length,ucc.comments,atc.nullable,atc.data_default from sys.all_tab_columns atc inner join sys.ALL_COL_COMMENTS ucc on atc.table_name=ucc.table_name and atc.column_name=ucc.column_name where atc.table_name = :tablename and atc.OWNER=:schema order by COLUMN_ID";
					...
					$sql="select atc.column_name,atc.data_type,data_length,ucc.comments,atc.nullable,atc.data_default from sys.user_tab_columns atc inner join sys.USER_COL_COMMENTS ucc on atc.table_name=ucc.table_name and atc.column_name=ucc.column_name where atc.table_name = :tablename order by COLUMN_ID";
	...
	//}
		...
		foreach ($desc['data'] as $field=>$fieldVal){
			...
			if (empty($fieldVal['notnull']) && $value===null && \key_exists($upperField,$arrays)){
				$strcolname[]=$field;
				$strvalues[]='NULL';
				...
				switch ($fieldVal['type']){
					...
					case 'longtext':
						...
						if (!@$fieldVal['typerange']){
							$ranges=array('tinytext'=>256,'text'=>65535,'mediumtext'=>16777215,'longtext'=>4294967295);
							if(isset($ranges[$fieldVal['type']]))$fieldVal['typerange']=$ranges[$fieldVal['type']];
						}
						...
						$bind[$upperField]=\is_float($value)?\number_format($value,0,".",""):$value;
						...
						if($value!==''){
							if(!\preg_match('#^-?\d+$#',$value)){
								...
								break;
                                                        ...
                                                        $maxMap=array('tinyint'=>127,'smallint'=>32767,'mediumint'=>8388607,'int'=>2147483647,'bigint'=>9223372036854775807);
							$max=isset($maxMap[$fieldVal['type']])?$maxMap[$fieldVal['type']]:null;
		...
		if ($outErrors){
			...
			if (core::$debug){
				debug::consoleLog('Have '.\count($outErrors).' errors. Break',error::ERROR);
				debug::dir($outErrors);
				debug::groupEnd();
			}
			return false;
...
//			db::e($sql,array('table_name'=>$table,'key'=>$key,'val'=>$item,'ip'=>request::ip(),'login'=>$login,'id'=>$id,'descr'=>isset($descriptions[$key])?$descriptions[$key]:$item));
	...
	static function delData($tablename,$arrayIn=array(),$db=''){
		if (\count($arrayIn)==0)return false;
		$bind=array();
		$strToDel=array();
		foreach($arrayIn as $key=>$value){
			$bind[$key]=$value;
