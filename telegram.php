...
class telegram{
	...
	static function send($method,$chat_id,$data=array()){
	    ...
	    if (isset($data['text']) && \is_string($data['text']))$data['text']=input::iconv($data['text'],true);
	    if (isset($data['caption']) && \is_string($data['caption']))$data['caption']=input::iconv($data['caption'],true);
	    if (self::$parse_mode && !isset($data['parse_mode']))$data['parse_mode']=self::$parse_mode;
	    ...
	    if (isset($data['reply_markup'])){
			if (is_array($data['reply_markup'])){
				$data['reply_markup']=\json_encode($data['reply_markup']);
			}
			unset($data['inline_keyboard'],$data['keyboard'],$data['resize_keyboard'],$data['one_time_keyboard'],$data['remove_keyboard'],$data['force_reply']);
	    }else{
	...
	static function editDocument($chat_id,$message_id,$link,$caption=null,$reply_markup=null){
	    return self::send('editMessageMedia',$chat_id,\array_filter([
		'message_id'=>$message_id,
		'media'=>\json_encode(\array_filter([
		    'type'=>'document',
		    'media'=>\is_file($link)?'attach://document':$link,
		    'caption'=>$caption,
		    'parse_mode'=>self::$parse_mode?:null
		])),
		'document'=>\is_file($link)?$link:null,
	...
	static function sendMediaGroup($chat_id,$mediaArray,$caption=null,$protect_content=false){
	    foreach ($mediaArray as $k=>&$v){
			if (\is_string($v))$v=array('type'=>'photo','media'=>$v);
	    }
	    if ($caption && isset($mediaArray[0])){
			$mediaArray[0]['caption']=input::iconv($caption,true);
			if (self::$parse_mode)$mediaArray[0]['parse_mode']=self::$parse_mode;
	    }
	...
	static function checkAuth($auth_data){
		...
		return $auth_data['auth_date']>=\time()-86400;
	}
}